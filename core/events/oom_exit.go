...
const (
	oomCaptureCmdlineOK      = 1 << 0
	oomCaptureCmdlineTrunc   = 1 << 1
	oomCaptureEnvironOK      = 1 << 2
	oomCaptureEnvironTrunc   = 1 << 3
	oomExitContextTTL        = 5 * time.Second
	oomExitContextWait       = oomExitContextTTL
...
func (c *oomExitEventCache) store(evt *abi.OOMExitEvent) {
	key := oomExitKey{pid: evt.PID, timestamp: evt.Timestamp}
	context := &oomExitContext{capturedAt: time.Now()}
	if evt.CaptureFlags&oomCaptureCmdlineOK != 0 {
		context.cmdline = strings.Join(
			decodeExitNUL(evt.VictimCmdline[:], evt.CmdlineLen), " ")
		context.cmdlineTruncated =
			evt.CaptureFlags&oomCaptureCmdlineTrunc != 0
	}
	if evt.CaptureFlags&oomCaptureEnvironOK != 0 {
		context.environ = decodeExitNUL(
			evt.VictimEnviron[:], evt.EnvironLen)
		context.environTruncated =
			evt.CaptureFlags&oomCaptureEnvironTrunc != 0
	}

	c.mu.Lock()
	defer c.mu.Unlock()

	c.purgeLocked(context.capturedAt)
	...
	entry.expiresAt = context.capturedAt.Add(oomExitContextTTL)
}
 ...
 */
func (c *oomExitEventCache) wait(ctx context.Context, pid uint32,
	timestamp uint64,
) *oomExitContext {
	if timestamp == 0 {
		return nil
	...
	key := oomExitKey{pid: pid, timestamp: timestamp}
	now := time.Now()
	c.mu.Lock()
	c.purgeLocked(now)
	entry, ok := c.entries[key]
	...
	timer := time.NewTimer(oomExitContextWait)
	defer timer.Stop()
	select {
	case <-ready:
	case <-timer.C:

...
func TestOOMExitEventCacheCorrelatesAndDecodes(t *testing.T) {
	...
	event := &abi.OOMExitEvent{
		PID:          42,
		Timestamp:    101,
		CaptureFlags: oomCaptureCmdlineOK | oomCaptureEnvironOK,
	}
...
func TestOOMExitEventCachePreservesTruncation(t *testing.T) {
	...
	event := &abi.OOMExitEvent{
		PID:       7,
		Timestamp: 202,
		CaptureFlags: oomCaptureCmdlineOK |
			oomCaptureCmdlineTrunc |
			oomCaptureEnvironOK |
			oomCaptureEnvironTrunc,
	}
...
func TestOOMExitEventCacheWakesConcurrentWaiters(t *testing.T) {
	cache := newOOMExitEventCache()
	key := oomExitKey{pid: 9, timestamp: 303}
	canceled, cancel := context.WithCancel(context.Background())
	cancel()

...
func NewELFSymbolizer(executable string, loadBias uint64) (*ELFSymbolizer, error) {
	...
	if textSection == nil {
		return nil, fmt.Errorf("executable %q has no text section", executable)
	}
	pcln, err := readPCLN(file)
	if err != nil {
		return nil, fmt.Errorf("read Go pclntab from %q: %w", executable, err)
	}
	var symtab []byte
	...
	table, err := gosym.NewTable(symtab, gosym.NewLineTable(pcln, textSection.Addr))
	if err != nil {

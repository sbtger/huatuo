...
func TestInspectCurrentGoProcess(t *testing.T) {
	...
	target, _, err := discoverer.inspectPID(os.Getpid(), os.Getpid())
...
func TestDiscoverPIDIncludesTargetProcess(t *testing.T) {
	...
	target, err := NewProcDiscoverer("/proc").DiscoverPID(context.Background(), pid)
	if err != nil {
		t.Fatalf("DiscoverPID: %v", err)
	}
	if target.TGID != uint32(pid) || target.StartTimeTicks == 0 ||
		target.MBucketsAddress() == 0 {
		t.Fatalf("incomplete target: %+v", target)
	}
}

...
const maxMProfFlushCodeSize = 512
...
func lookupStrippedMBuckets(file *elf.File) (uint64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("%w: %v", errMBucketsSymbolNotFound, err)
	}
	...
	if err != nil {
		return 0, fmt.Errorf("%w: read runtime.mProf_FlushLocked: %v",
...
func decodeAMD64MBuckets(
	...
	for offset := 0; offset < len(code); {
		...
		if err != nil || instruction.Len == 0 {
			return 0, fmt.Errorf("decode amd64 instruction at %#x: %v", entry+uint64(offset), err)
		}

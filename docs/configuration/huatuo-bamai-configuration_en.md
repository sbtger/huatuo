...
#### 8.7 OOM Go Heap Profiling (EventTracing.OOMGoHeap)

This optional path snapshots the Go runtime heap immediately before the kernel delivers `SIGKILL` to an OOM victim. It stores in-use space and in-use object profiles with the same `tracer_id` as the base OOM event. Loading, conversion, or storage failures disable or skip only heap profiling; base OOM collection continues.
...
- **Enabled**: Enables the optional heap path. Default: `false`.
- **CaptureBudgetMicroseconds**: Maximum time spent by one pre-exit BPF capture. Valid range: 1–10000 μs. Default: 2000 μs.
- **ReconcileIntervalSeconds**: Interval for discovering profileable Go processes. Must be greater than zero. Default: 10 seconds.
- **MaxTargets**: Maximum registered Go processes. Valid range: 1–4096. Default: 4096.

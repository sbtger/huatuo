...
|loadavg_nr_uninterruptible|Host uninterruptible tasks contributing to load|count|Host| host, region |Shared BPF task iterator, opt-in|
...
|loadavg_container_load1|Estimated 1-minute container R+D load average|count|Container| host, region |cgroup v1/v2, configurable EMA sampling (default: 15 seconds)|
|loadavg_container_load5|Estimated 5-minute container R+D load average|count|Container| host, region |cgroup v1/v2, configurable EMA sampling (default: 15 seconds)|
|loadavg_container_load15|Estimated 15-minute container R+D load average|count|Container| host, region |cgroup v1/v2, configurable EMA sampling (default: 15 seconds)|
...
Host `loadavg_nr_uninterruptible` is opt-in through `MetricCollector.Loadavg.EnableHostUninterruptible=true` (default false). It reuses dload's shared BPF task iterator at the configured loadavg interval (default: 15 seconds) to count host D-state tasks contributing to load, excluding `TASK_NOLOAD`/`TASK_FROZEN`. It includes non-container tasks, is not a sum of container metrics, and does not use `/proc/stat`'s `procs_blocked`. Enabled v2 container statistics share the same traversal. Host collection works independently of cgroup v1/v2 mode but requires kernel BTF, the BPF task iterator and the host PID namespace. Unsupported systems omit the metric without a procfs task-scan fallback; existing host loadavg and v1 container metrics remain available. Counts are sampled during traversal, not an atomic snapshot of all tasks.
